    ...
    for y in range(height):
        for x in range(width):
            if processed_map[y][x] == '#':
                direction, magnitude = get_direction_and_magnitude(src_x=src_x, src_y=src_y, dst_x=x, dst_y=y)
...
def find_optimal_monitoring_post_location(processed_map: List[str]) -> Tuple[Tuple[int, int], int]:
    ...
    for y in range(height):
        for x in range(width):
            if processed_map[y][x] == '#':
                distances_dict = generate_distances_dict(processed_map, x, y)
                visible_count = get_count_of_unique_vectors(distances_dict)
...
def run():
    ...
    processed_map: List[str] = read_map(image_stream)
    best_location, best_count = find_optimal_monitoring_post_location(processed_map)

    # part 2
    new_distances_dict: Dict[Tuple[int, int], Tuple[float, float]] = \
        generate_distances_dict(processed_map, best_location[0], best_location[1])
    atan2_rotate_value = pi / 2
    rotated_switched_dict = rotate_switch_dictionary(new_distances_dict, atan2_rotate_value)
    fire_laser(rotated_switched_dict, 200)
